...
public class AtmController {
    ...
    @ResponseBody
    public ResponseEntity<?> addNewAtm(@RequestBody final AtmDetail atmDetail) {
        try {
            ...
            if (atmServiceException.getCause().getClass().equals(AtmAlreadyTakenException.class)) {
                return new ResponseEntity<>(new CustomErrorType("Atm Already Taken to  AccountNumber  " + atmDetail.getAccNumber()), HttpStatus.ALREADY_REPORTED);
            }
            else if (atmServiceException.getCause().getCause().getClass().equals(AccountDaoException.class)) {
                return new ResponseEntity<>(new CustomErrorType("Invalid AccountNumber  " + atmDetail.getAccNumber()), HttpStatus.NOT_FOUND);
            }
            return new ResponseEntity<>(new CustomErrorType("issue atm to customer Failed" + atmServiceException.getCause().getCause()), HttpStatus.EXPECTATION_FAILED);
        }
    }
    ...
    @ResponseBody
    public ResponseEntity<?> displayAllAtm() {
    ...
    @ResponseBody
    public ResponseEntity<?> checkAtmIssue(@RequestParam("accNumber") final int accNumber) {
        List<AtmDetail> atmDetails = null;
        try {
            atmDetails = atmService.isAlreadyAtmTaken(accNumber);
        ...
        return new ResponseEntity<>("Atm Already Taken Details:" + atmDetails, HttpStatus.OK);
    }
