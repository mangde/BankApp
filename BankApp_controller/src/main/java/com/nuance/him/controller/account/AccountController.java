...
public class AccountController {
    ...
    @ResponseBody
    public ResponseEntity<?> addNewAccount(@Valid @RequestBody final Account account) {
        try {
    ...
    @ResponseBody
    public ResponseEntity<?> getCurrentBalance(@RequestParam("accNumber") final int accNumber) {
        try {
            ...
            return new ResponseEntity(new CustomErrorType("current balance exception account Id Not found " + accountServiceException.getCause().getMessage()), HttpStatus.NOT_FOUND);
        }
    }
     ...
     */
    @RequestMapping(value = DEPOSITE_AMOUNT, method = RequestMethod.PUT)
    @ResponseBody
    public ResponseEntity<?> deposite(@RequestParam("accNumber") final int accNumber, @RequestParam("amount") final double amount) {
        try {
            ...
            return new ResponseEntity(new CustomErrorType("Failed to Deposite balance " + accountServiceException.getCause().getMessage()), HttpStatus.EXPECTATION_FAILED);
        }
    }
    ...
    @ResponseBody
    public ResponseEntity<?> getAccountDetails(@RequestParam("accNumber") final int accNumber) {
        try {
    ...
    @ResponseBody
    public ResponseEntity<?> withDrawAmount(@RequestParam("accNumber") final int accNumber, @RequestParam("amount") final double amount) {
        try {
