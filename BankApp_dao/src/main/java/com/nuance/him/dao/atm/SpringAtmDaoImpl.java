...
public class SpringAtmDaoImpl implements SpringAtmDao {
    ...
    public AtmDetail issueAtmCard(final AtmDetail atmDetail) throws AtmDaoException {
        try {
            System.out.println("isseatm dao layer");
            ...
            System.out.println("isseatm dao layer after query");
    ...
    @Override
    public List<AtmDetail>  isAlreadyAtmTaken(final int accNumber) throws AtmDaoException {
        final SqlParameterSource namedParameters = new MapSqlParameterSource("accNumber", accNumber);
        try {
            return namedParameterJdbcTemplate.query(isAlreadyAtmTakenQuery, namedParameters, new AtmMapper());
        }
        catch (final DataAccessException dataAccessException) {
